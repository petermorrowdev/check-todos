...
def iter_todos(path, pattern):
    ...
    for comment_node in captures.get('comments', []):
        ...
        if is_todo or is_fixme:
            context = []
            for no, line in source_lines[max(0, start_line_no - 2) : end_line_no + 3]:
...
def main():
    ...
    parser.add_argument(
        ...
        nargs='*',
        help=(
            'glob of source code files to search for TODOs in or when missing '
            'defaults to `git ls-files`.'
        ),
    )
    parser.add_argument(
        '--output',
    ...
    if args.path:
        for glob in args.path:
            ...
            for path in paths:
                for todo in iter_todos(path, pattern):
                    if todo.issue:
        ...
        if not git_path:
            print(red_bold('path required when git is unavailable'))
            sys.exit(1)

        result = subprocess.run(  # noqa: S603 # git_path derived from which
            [git_path, 'ls-files'],
            text=True,
            capture_output=True,
            check=True,
